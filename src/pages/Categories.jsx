...
const Categories = () => {
  ...
  const handleConfirmUpdate = async () => {
    try {
      await axios.put(`http://localhost:8080/api/v1/category/${updateId}/update`, {
        name: updateName,
      });
  ...
  const handleConfirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:8080/api/v1/category/${deleteId}/delete`);
  ...
  return (
    <Box sx={{ p: 2, m: 0, width: '100%' }}>
        ...
        <Table>
          <TableHead sx={{ backgroundColor: '#2563EB' }}>
            <TableRow>
              ...
              <TableCell sx={{ color: 'white', fontSize: '1.2rem' }}>Tên Loại</TableCell>
              ...
              <TableRow key={cat.id}>
                ...
                <TableCell>
                  <Box sx={{ display: 'flex', gap: 1 }}>
                    <Button variant="contained" color="primary" size="small" onClick={() => handleOpenUpdate(cat)}>
                      Cập nhật
                    </Button>
                    <Button variant="contained" color="error" size="small" onClick={() => handleOpenDelete(cat.id)}>
                      Xóa
                    </Button>
                  </Box>
                </TableCell>
      ...
      <Dialog open={openDelete} onClose={handleCloseDelete}>
        ...
        <DialogContent>Bạn có chắc chắn muốn xóa loại sản phẩm này không?</DialogContent>
        <DialogActions>
